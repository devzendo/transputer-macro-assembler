...
class TestBinaryWriter extends TempFolder with AssertionsForJUnit with MustMatchers {
    ...
    def fileSizeAndContents(): Unit = {
        ...
        model.allocateStorageForLine(IndexedLine(1, SourceLocation("", 2), "", None, Some(DB(exprs1))), 1, exprs1)

        model.setDollar(0x4020, genDummyLine(3))

        val exprs2 = List(Number(3), Number(4))
        model.allocateStorageForLine(IndexedLine(3, SourceLocation("", 4), "", None, Some(DB(exprs2))), 1, exprs2)
    ...
    def littleEndianWord(): Unit = {
        ...
        model.allocateStorageForLine(IndexedLine(0, SourceLocation("", 1), "", None, Some(new DW(words))), 2, words)
